...
public class UctNode {
    ...
    Traxboard getPosition () { return position; }
    int getWins() { return wins; }
    int getDraws() { return draws; }
    int getVisits() { return visits; }
    UctNode getParent () { return parent; }
    ArrayList<UctNode> getChildren() { return children; }
    String getMove() { return move; }

    UctNode getWorse() {
	assert(children.size()>0);	/* Don't call this method for nodes without children created */
	int worse_index=0;
	...
	return (getWinrate()+
		10*(float)Math.sqrt(Math.log(getParent().getVisits())/(5*getVisits ())));
    }
