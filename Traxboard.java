...
/**
 * Returns the numbers of used tiles. 
 * Can be used to determine if we are in the opening, middle or  
...
/**
 * Given a row and column, the method checks if that (row,col) is occupied or free
 ...
 * update the board.
 * Accepts upper-case and lower-case letters,  old and new notation 
 * but not the very old notation (used until 1986?) 
 ...
 */
  public void makeMove (String move) throws IllegalMoveException
  {
	...
	throw new
	  ...
	  ("Only A1C,A1S,@0/ and @0+ accepted as first move. Got " + move);
      }
  ...
  public ArrayList < String > uniqueMoves ()
  {
    ...
    int ohs_up, ohs_down, ohs_right, ohs_left, eks_up, eks_down, eks_right,
      eks_left;
    ...
    if (boardEmpty == true)
      {				//empty board only these two moves
	Moves.add ("@0/");
	Moves.add ("@0+");
	Moves.trimToSize ();
	return Moves;
  ...
  public boolean forcedMove (int brow, int bcol)
  {
    ...
    int white_up = 0, black_up = 0, white_down = 0, black_down = 0,
      white_left = 0, black_left = 0, white_right = 0, black_right = 0;
	...
	switch (direction)
	  ...
	  case 'd':
		...
		/*
		   ...
		   icopy=i; 
		   jcopy=j;
		   updateLine('w','w',icopy,jcopy);
		   wnum[i][j]=currentWNum;
		   wnum[icopy][jcopy]=currentWNum;
		   currentWNum++;
		   }
		   snprintf(dummy,4,"%d",wnum[i][j]);
		   result+=dummy;
		   }
		 */
		break;
		...
		/*
		   ...
		   icopy=i; 
		   jcopy=j;
		   updateLine('b','w',icopy,jcopy);
		   bnum[i][j]=currentBNum;
		   bnum[icopy][jcopy]=currentBNum;
		   currentBNum++;
		   }
		   result+=(char)bnum[icopy][jcopy];
		   snprintf(dummy,4,"%d",bnum[i][j]);
		   result+=dummy;
		   }
		 */
		break;
